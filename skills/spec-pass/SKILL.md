...
purely mechanical and are handled entirely by `spec-loop` itself before
this skill is ever invoked; this skill only carries an already-claimed
child, or a child already sitting in `review/`, to its terminal state
per [TICKET-FORMAT.md](../agent-tickets/TICKET-FORMAT.md)'s "Spec board
operations". This skill never moves or commits the ticket into
`agent-tickets` itself, no matter how mechanical that move now is —
each mode only reports the fate it decided back to whoever invoked it
(see "Report fate" in TICKET-FORMAT.md). Enacting it is the caller's
job alone.
...
there — do not move or commit the ticket yourself, even though you now
have everything needed to; that move belongs to the caller.
...
5. **Decide its fate** — report it and stop there; do not move or
   commit the ticket yourself, even though you now have everything
   needed to — that move belongs to the caller:
