...
description: Generate exhaustive, non-negotiably rigorous black-box test case descriptions (Given/When/Then, not code) for a public API's external contract — never its implementation. Use for zero-tolerance test coverage, an extremely strict/exhaustive black-box test case sweep, or whenever "thorough" test-case generation isn't thorough enough. Produces an API handoff file, one case file per sub-agent — each is its own source of truth — and a master index file pointing at all of them. Every case carries a checkbox so a later implementing agent can track what's been turned into runnable tests. This skill never writes runnable tests itself.
...
1. **The implementation is never read by any case-authoring agent — this is
   ...
   - Exactly one sub-agent (Stage 1, the handoff sub-agent) reads implementation
     source. Every other agent in this pipeline — fan-out, interaction-pass, and
     the orchestrator itself — works only from text handed to it in its own
     prompt, and must never invoke Read/Grep/Glob/Bash against implementation
     files.
...
4. **Stage 2 — Fan-out, one `Agent` (subagent_type `general-purpose`) per
   method, in parallel.** Each spawn receives only: that method's handoff entry,
   plus the full `### <TypeName>` entries for every type in its `References:`
   line, plus the file-naming and templates below. Explicit instructions:
...
5. **Stage 2b — Interaction-pass agent**, one `Agent` (subagent_type
   `general-purpose`), full handoff as input, dedicated to cross-method/stateful
   sequence cases per Standard 4. Same self-review, same direct-write, same
   compact-summary-only return, writing to `<Target>.cases.interactions.md`.
   Skip only if the handoff has a single method with no state — name that
   justification explicitly in the final report if skipped.
...
**Generated by:** the `blackbox-test-cases` skill
(`~/.claude/skills/blackbox-test-cases/SKILL.md`) — read that file for the full
pipeline, file-naming, and status-tracking conventions this index and its
linked case files follow.
...
- [ ] `<Target>.cases.<slug>.md` — <method> (0/<n>)
...
|---|---|---|---|---|
| <method> | `<Target>.cases.<slug>.md` | <n> | 0 | <gap summary, or "none"> |
